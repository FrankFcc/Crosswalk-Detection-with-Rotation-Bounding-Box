...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1 = make_last_layers(darknet.output, 512, num_anchors * (num_classes + 6))

    x = compose(DarknetConv2D_BN_Leaky(256, (1, 1)), UpSampling2D(2))(x)
    x = Concatenate()([x, darknet.layers[152].output])
    x, y2 = make_last_layers(x, 256, num_anchors * (num_classes + 6))

    x = compose(DarknetConv2D_BN_Leaky(128, (1, 1)), UpSampling2D(2))(x)
    x = Concatenate()([x, darknet.layers[92].output])
    x, y3 = make_last_layers(x, 128, num_anchors * (num_classes + 6))
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    feats = K.reshape(
        feats, [-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 6]
    )
    ...
    # tf.print(feats[..., 5:6], output_stream=sys.stdout, )
    box_confidence = K.sigmoid(feats[..., 4:5])
    box_degree = K.sigmoid(feats[..., 5:6])
    box_class_probs = K.sigmoid(feats[..., 6:])
    if calc_loss == True:
        return grid, feats, box_xy, box_wh, box_degree

    return box_xy, box_wh, box_confidence, box_class_probs, box_degree


def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, box_degree):
    """Get corrected boxes"""
    box_yx = box_xy[..., ::-1]
    box_hw = box_wh[..., ::-1]
    ...
    boxes = K.concatenate([boxes, box_degree])
    return boxes


def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    """Process Conv layer output"""
    box_xy, box_wh, box_confidence, box_class_probs, box_degree = yolo_head(
        feats, anchors, num_classes, input_shape
    )
    # print(box_degree)
    # tf.print(box_degree, output_stream=sys.stdout, )
    boxes = yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, box_degree)
    boxes = K.reshape(boxes, [-1, 5])
...
def yolo_eval(
    ...
    # tf.print(yolo_outputs, output_stream=sys.stdout, )
    anchor_mask = (
    ...
    for c in range(num_classes):
        ...
        nms_index = tf.image.non_max_suppression(
            class_boxes[:, 0:4], class_box_scores, max_boxes_tensor, iou_threshold=iou_threshold
        )
        ...
        for t, n in enumerate(best_anchor):
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    ...
                    y_true[l][b, j, i, k, 5] = true_boxes[b, t, 5].astype("int32")
                    y_true[l][b, j, i, k, 6 + c] = 1
                    # tf.print(y_true[l][b, j, i, k], output_stream=sys.stdout, )
    return y_true
...
def skew_bbox_iou(box2, box1):
    ...
    print(box1)
    print(box2)
    box2 = K.expand_dims(box2, -2)
    box1 = K.expand_dims(box1, 0)

    box1 = box1[..., :5]
    box2 = box2[..., :5]
    print(box1)
    print(box2)
    K.print_tensor(box1, message='box1: ', summarize=-1)
    K.print_tensor(box2, message='box2: ', summarize=-1)

    ious = []
    for i in range(box2.shape[3]):
        r_b1 = get_rotated_coors(box1)
        r_b2 = get_rotated_coors(box2[i])

        ious.append(skewiou(r_b1, r_b2))

    return tf.convert_to_tensor(ious)
...
def get_rotated_coors(box):
    ...
    cx = box[..., 0]; cy = box[..., 1]; w = box[..., 2]; h = box[..., 3]; a = math.radians(box[..., 4])
    ...
    if isinstance(x0, tf.Tensor):
        r_box=K.concatenate([x0.unsqueeze(0),y0.unsqueeze(0),
                         x1.unsqueeze(0),y1.unsqueeze(0),
                         x2.unsqueeze(0),y2.unsqueeze(0),
                         x3.unsqueeze(0),y3.unsqueeze(0)])
    else:
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=0.5):
    ...
    for l in range(num_layers):
        ...
        true_degree = y_true[l][..., 5:6]
        true_class_probs = y_true[l][..., 6:]

        grid, raw_pred, pred_xy, pred_wh, pred_degree = yolo_head(
            yolo_outputs[l],
            anchors[anchor_mask[l]],
            num_classes,
            input_shape,
        ...
        def loop_body(b, ignore_mask):
            ...
            # tf.print(true_box, output_stream=sys.stdout, )
            iou = box_iou(pred_box[b], true_box)
            # iou = skew_bbox_iou(pred_box[b], true_box)
        ...
        class_loss = object_mask * K.binary_crossentropy(
            true_class_probs, raw_pred[..., 6:], from_logits=True
        )
        # tf.print(raw_pred[..., 6:], output_stream=sys.stdout, summarize=-1)
        # degree_loss = object_mask * tf.reduce_mean(tf.abs(tf.atan2(tf.sin(true_degree - pred_degree), tf.cos(true_degree - pred_degree))))
        degree_loss = K.abs(
                object_mask
                * 0.2
                * K.binary_crossentropy(true_degree, pred_degree, from_logits=True)
        )
        # degree_loss = object_mask * mean_squared_error_360(true_degree, pred_degree)
        xy_loss = K.sum(xy_loss) / mf
        wh_loss = K.sum(wh_loss) / mf
        confidence_loss = K.sum(confidence_loss) / mf
        class_loss = K.sum(class_loss) / mf
        degree_loss = K.sum(degree_loss) / mf
        xy_loss = K.print_tensor(xy_loss, message='xy_loss = ')
        wh_loss = K.print_tensor(wh_loss, message='wh_loss = ')
        ...
        degree_loss = K.print_tensor(degree_loss, message='degree_loss = ')
        loss += xy_loss + wh_loss + confidence_loss + class_loss + degree_loss
