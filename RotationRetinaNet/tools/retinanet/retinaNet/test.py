...
from libs.utils.coordinate_convert import backward_convert, get_horizen_minAreaRectangle, our_convert
...
class TrainRetinaNet(Train):
    ...
    def get_gtboxes_and_label(self, gtboxes_and_label_h, gtboxes_and_label_r, num_objects):
        return gtboxes_and_label_h[:int(num_objects), :].astype(np.float32), \
               gtboxes_and_label_r[:int(num_objects), :].astype(np.float32) # for chioce
        ...
        with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
            ...
            for i in range(num_gpu):
                ...
                gtboxes_and_label_r = tf.py_func(backward_convert,
                                                 ...
                                                 Tout=tf.float32)
                gtboxes_and_label_ours = tf.py_func(our_convert,
                                                 inp=[gtboxes_and_label_r[i]],
                                                 Tout=tf.float32)
                gtboxes_and_label_ours = tf.reshape(gtboxes_and_label_ours, [-1, 7])
...
if __name__ == '__main__':
    ...
    trainer = TrainRetinaNet(cfgs_res50_dota_v15)
